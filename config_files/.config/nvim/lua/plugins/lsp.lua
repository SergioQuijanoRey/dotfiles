return {
	-- Builtin lsp
	{
		...
		config = function()
			...
			setmap("n", "<leader>qs", function()
				vim.lsp.buf.signature_help({ border = "rounded" })
			end, bufopts, "Signature help")
			setmap("n", "<leader>qh", function()
				vim.lsp.buf.hover({ border = "rounded" })
			end, bufopts, "Hover (docs)")
			...
			-- List of servers that I want to have access to with default configuration
			local servers = {
				...
				-- For Rust
				-- NOTE: configured with clippy in `lpst/rust_analyzer.lua`
				"rust_analyzer",
